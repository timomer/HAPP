//#!/usr/bin/env node     TODO Tim: Causes crash, had to comment out
...
function run(glucose_input,temps_input,iob_input,profile_input,offline_value) {
    ...
    //    java.lang.System.out.println('usage: ', process.argv.slice(0, 2), '<iob.json> <current-temps.json> <glucose.json> <profile.json> [Offline]');
    //    process.exit(1);
    //}
    ...
    if (typeof profile_data === 'undefined' || typeof profile_data.current_basal === 'undefined') {
        print('Error: could not get current basal rate');
        process.exit(1);
    }
    ...
    if (typeof profile_data.target_bg !== 'undefined') {
        ...
        if (typeof profile_data.max_bg !== 'undefined' && typeof profile_data.max_bg !== 'undefined') {
            ...
            print('Error: could not determine target_bg');
            process.exit(1);
        }
    }
    ...
    } else { java.lang.System.out.println("Could not determine last BG time"); }
                ...
                if ((glucose_status.delta > 0 && eventualBG < profile_data.min_bg) || (glucose_status.delta < 0 && eventualBG >= profile_data.min_bg)) {
                    ...
                    if (basal_iob > max_iob) {
                        ...
                        if (rate > maxSafeBasal) {
                            rate = maxSafeBasal;
                            //java.lang.System.out.println(maxSafeBasal);
                        }
...
java.lang.System.out.println(JSON.stringify(requestedTemp));

    return JSON.stringify(requestedTemp);
}
